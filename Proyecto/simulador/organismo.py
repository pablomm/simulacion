...
class OrganismoSencillo(Organismo):
    ...
    def __init__(self, r_explotacion=1, posicion=None, stop_eat=False,
        ...
        super().__init__(r_explotacion=r_explotacion, posicion=posicion, name=name)
...
class RandomWalker(OrganismoSencillo):
    ...
    def __init__(self, r_explotacion=1., std=1., stop_eat=False, posicion=None):
        ...
        super().__init__(r_explotacion=r_explotacion, stop_eat=stop_eat, posicion=posicion,
                         name="Random Walker")
...
class OrganismoVFija(Organismo):
    ...
    def __init__(self, velocidad=1., r_explotacion=1, r_sensibilidad=0., posicion=None,
        ...
        super().__init__(r_explotacion=r_explotacion, r_sensibilidad=r_sensibilidad, posicion=posicion, name=name)
...
class Organismo2Etapas(OrganismoVFija):

    def __init__(self, r_explotacion=1., r_sensibilidad=0., velocidad=1., mu=0.,
                std=1., a=1.5, b=0., loc=0., scale=1., maximo=np.Inf, minimo=0,
                stop_eat=False, posicion=None):
        ...
        self.tipo_movimiento = 0 #Levy flight (0) por defecto, random walker (1)

        super().__init__(r_explotacion=r_explotacion, r_sensibilidad=r_sensibilidad,
                        posicion=posicion, name="Organismo 2 Etapas")
    ...
    def step(self):
        ...
        if not self.stop_eat or len(indices) == 0:
            ...
            if len(indices_deteccion)>0 and self.tipo_movimiento==0:
                self.tipo_movimiento = 1 #Cambiamos a random walker
                self.generar_movimiento()
            elif len(indices_deteccion) == 0 and self.tipo_movimiento==1:
                self.tipo_movimiento = 0 #Cambiamos a Levy flight
                self.generar_movimiento()
    ...
    def generar_movimiento(self):
        self.theta = np.random.uniform(0,2*np.pi)
        if self.tipo_movimiento==1: #Random walker
            self.remaining = np.linalg.norm(np.random.normal(scale=self.std,
                                                             loc=self.mu, size=2))
        else:
            d = st.levy_stable.rvs(self.a, self.b, self.loc, self.scale)
